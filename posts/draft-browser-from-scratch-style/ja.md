...
ブラウザ自作を始めた時点での、周辺技術に関する私の習熟度についても書いておく。

まず Rust について。Rust は好きな言語の一つで、仕事でちょっと書いたりしている。Rust の中でも safe Rust, async Rust は書いたことがあるが、unsafe Rust[^2] は書いたことがない。`Rc` とか `RefCell` とかもよくわからない。

次にブラウザについて。Web セキュリティを趣味や仕事でやってきたし、今の仕事は SaaS の開発なので、「つかう側」としてのブラウザへの知識は、まあそれなりにあると思っている。一方、ブラウザ自体の開発へのコントリビュートなどはしたことがない。ただ、ブラウザ自作に一度チャレンジしたことはある。「[ちいさな Web ブラウザを作ろう](https://browserbook.shift-js.info/)」に取り組んでいた (この時も、分割キーボードに慣れようとしていた)。ページの描画まではうまくいったのだが、JavaScript エンジンを組み込むところで所有権バトルに敗北してしまった[^1]。


[^1]: 所有権バトルに敗北した要因は当該資料ではなく、我流に走ったためである。公開からしばらく経った資料なので、使用されているライブラリの多くに新しいバージョンが出ているのだが、なんとなく新しいバージョンを選んでしまった。すると各種関数のシグネチャが変わっているので、資料通りにやってもコンパイルエラーが出る。正直なところ、講義資料で使われているライブラリのバージョンに従うことを推奨する。ブラウザの仕組みを学ぶという目的のもとで、ライブラリのアップデートに伴うシグネチャの変化への対応は本質ではないからだ。

[^2]: sababook で unsafe Rust を書く機会はなさそうだ。
