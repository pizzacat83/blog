...
ただ、テンプレートファイルの中で補完が効くわけではない。Askama の IntelliJ Plugin はあるのだが、テンプレートファイルを編集する際に補完や型エラーのフィードバック、jump to definition などはできなかった (私の環境構築がうまくいっていないのかもしれない)。Rust の型システムの恩恵を最大限受けられるわけではないと感じた。
